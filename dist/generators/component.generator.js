import { toKebabCase, toAngularSelector, toCamelCase, } from '../utils/naming.utils.js';
import { mapReactTypeToAngular } from '../utils/type-mapper.utils.js';
export function generateAngularComponent(analysis, moduleName) {
...
function generateComponentTs(analysis, selector, kebabName) {
    ...
    if (hasOnInit)
        lifecycleInterfaces.push('OnInit');
    if (hasOnDestroy)
...
function buildImports(analysis) {
    const angularImports = new Set([
        'Component',
...
function buildComputedSignals(analysis) {
    if (analysis.memos.length === 0)
        return '';
...
function buildEffects(analysis) {
    const lines = [];
    // Effects con dependencias → effect() de Signal
    const signalEffects = analysis.effects.filter((e) => !e.isOnMount && !e.isOnDestroy && e.dependencies.length > 0);
...
function buildMethods(analysis) {
    if (analysis.callbacks.length === 0)
        return '';
...
function generateComponentHtml(analysis) {
...
function transformJsxToAngular(jsx) {
    let template = jsx;
    // className → class
    ...
    template = template.replace(/value=\{(\w+)\}/g, '[ngModel]="$1()" (ngModelChange)="$1.set($$event)"');
    // checked={stateVar} en checkboxes → [ngModel]="stateVar()" (ngModelChange)="stateVar.set($event)"
    template = template.replace(/checked=\{(\w+)\}/g, '[ngModel]="$1()" (ngModelChange)="$1.set($$event)"');
    ...
    template = template.replace(/\{(\w+)\s*&&\s*(<[^>]+>[^<]*<\/[^>]+>)\}/g, '@if ($1) {\n  $2\n}');
    // {condition ? <a> : <b>} → @if / @else
    template = template.replace(/\{(\w+)\s*\?\s*(<[^>]+>[^<]*<\/[^>]+>)\s*:\s*(<[^>]+>[^<]*<\/[^>]+>)\}/g, '@if ($1) {\n  $2\n} @else {\n  $3\n}');
    // {items.map(item => <element>)} → @for con track
    template = template.replace(/\{(\w+)\.map\(\(?(\w+)\)?\s*=>\s*(<[^]*?)\)\}/g, '@for ($2 of $1; track $index) {\n  $3\n}');
...
function generateComponentScss(analysis) {
    const lines = [
...
function generateComponentSpec(analysis, kebabName) {
...
function generateSignalTests(analysis) {
    if (analysis.stateHooks.length === 0)
        return '';
    return analysis.stateHooks
        .map((hook) => {
...
function generateInputTests(analysis) {
    const inputProps = analysis.props.filter((p) => p.name !== '_propsInterface');
    if (inputProps.length === 0)
        return '';
    return inputProps
