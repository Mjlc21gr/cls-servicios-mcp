...
export function injectClsThemeToScss(rawScss) {
    let themed = rawScss;
...
export function injectClsThemeToHtml(html, uiLibraries) {
    let themed = html;
    ...
    if (uiLibraries.some((lib) => lib.includes('mui') || lib.includes('material'))) {
        for (const [muiComp, clsClass] of Object.entries(MUI_TO_CLS_MAP)) {
            ...
            themed = themed.replace(new RegExp(`<${muiComp}([\\s>])`, 'g'), `<div class="${clsClass}"$1`);
            themed = themed.replace(new RegExp(`</${muiComp}>`, 'g'), '</div>');
        }
    }
...
export function generateClsThemeVariables() {
...
//# sourceMappingURL=theme-injector.generator.js.map