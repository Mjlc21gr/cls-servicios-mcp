...
export class GitHubClient {
    ...
    async pullReactFiles() {
        ...
        try {
            ...
            for (const item of tree) {
                ...
                if (!item.path)
                    continue;
                if (!/\.(tsx|jsx)$/.test(item.path))
                    continue;
                // Ignorar archivos de test/stories
                if (/\.(test|spec|stories)\.(tsx|jsx)$/.test(item.path))
                    continue;
                const content = await this.getFileContent(`${basePath}/${item.path}`);
                if (content) {
                    files.push({
                        path: item.path,
                        content,
            ...
            throw new Error(`Error al traer archivos de ${this.config.owner}/${this.config.repo}@${this.config.branch}: ${error instanceof Error ? error.message : String(error)}`);
        }
        return files;
    }
    /**
     ...
     */
    async pushMigratedFiles(files, commitMessage) {
        try {
