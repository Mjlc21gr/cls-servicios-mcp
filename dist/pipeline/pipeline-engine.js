...
import { injectClsThemeToScss, injectClsThemeToHtml, } from '../generators/theme-injector.generator.js';
...
export class PipelineEngine {
    ...
    async stepAnalyzeReact() {
        ...
        return this.gate('analyze_react', true, `${this.context.analyses.length}/${this.context.reactFiles.length} componentes analizados`, [], errors.length > 0 ? errors.map((e) => `[warn] ${e}`) : []);
    }
    // ═══════════════════════════════════════════════════════════════
    // PASO 3: Validar Estructura
    // ═══════════════════════════════════════════════════════════════
    async stepValidateStructure() {
        const errors = [];
        const warnings = [];
        ...
        for (const analysis of this.context.analyses) {
            ...
            const fileSizeKb = Buffer.byteLength(this.context.reactFiles.find((f) => f.path === analysis.fileName)?.content ?? '', 'utf-8') / 1024;
    ...
    async stepInjectPrimeNg() {
        ...
        for (const [filePath, content] of Object.entries(this.context.generatedFiles)) {
            ...
            if (primeImports) {
                ...
                const updatedContent = content.replace("import { CommonModule } from '@angular/common';", `import { CommonModule } from '@angular/common';\n${primeImports}`);
                // Agregar módulos PrimeNG al array de imports del componente
                const moduleNames = [...new Set(allModules.map((m) => m.moduleName))];
                const updatedImports = updatedContent.replace('imports: [CommonModule]', `imports: [CommonModule, ${moduleNames.join(', ')}]`);
        ...
        return this.gate('inject_primeng_ui', true, `${convertedCount} templates convertidos a PrimeNG. Módulos: [${uniqueModules.join(', ')}]. Tema SB generado.`, [], warnings);
    }
    // ═══════════════════════════════════════════════════════════════
    // PASO 6: Inyectar Tema CLS
    // ═══════════════════════════════════════════════════════════════
    async stepInjectClsTheme() {
        let themedCount = 0;
        for (const [filePath, content] of Object.entries(this.context.generatedFiles)) {
            if (filePath.endsWith('.component.html')) {
    ...
    // PASO 11: GitHub Push
    // ═══════════════════════════════════════════════════════════════
    async stepGithubPush() {
        if (this.config.dryRun) {
            return this.gate('github_push', true, `[DRY RUN] Se enviarían ${Object.keys(this.context.generatedFiles).length} archivos a ${this.config.destRepo.owner}/${this.config.destRepo.repo}@${this.config.destRepo.branch}`);
        }
        ...
        return this.gate('github_push', true, `${Object.keys(this.context.generatedFiles).length} archivos enviados a ${this.config.destRepo.owner}/${this.config.destRepo.repo}@${migrationBranch} (commit: ${pushResult.commitSha?.substring(0, 7)})`);
    }
    // ═══════════════════════════════════════════════════════════════
    // Helpers
    // ═══════════════════════════════════════════════════════════════
    gate(step, passed, message, errors = [], warnings = []) {
        return {
            passed,
            step,
            message,
    ...
    failStep(step, errorMessage) {
        step.status = 'failed';
        step.gateResult = this.gate(step.id, false, errorMessage, [errorMessage]);
        this.execution.status = 'failed';
        this.execution.failedAtStep = step.id;
        this.skipRemainingSteps(step.order);
    }
    skipRemainingSteps(fromOrder) {
        for (const step of this.execution.steps) {
    ...
    createExecution() {
        const steps = PIPELINE_STEPS.map((id, index) => ({
            id,
