...
// ---------------------------------------------------------------------------
const IMPORT_PATH_MIGRATIONS = new Map([
 ...
 */
function removeRequiredFromPrimeNG(html) {
    let result = html;
    for (const tag of PRIMENG_NO_REQUIRED) {
        ...
        const tagPattern = new RegExp(`(<${escapeRegex(tag)}\\b[^>]*)\\s+(?:\\[?required\\]?)(?:="[^"]*")?(?=[\\s/>])`, 'g');
 ...
 */
function migrateTemplateTags(html) {
    let result = html;
 ...
 */
function ensureStandaloneImports(ts) {
    let result = ts;
 ...
 */
export function sanitizePrimeNG(components) {
...
//# sourceMappingURL=primeng-sanitizer.js.map