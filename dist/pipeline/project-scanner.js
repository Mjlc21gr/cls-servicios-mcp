...
export async function scanProject(sourceDir) {
    ...
    if (!existsSync(rootDir)) {
        throw new Error(`Source directory does not exist: "${rootDir}". Please provide a valid React project path.`);
    }
    const stat = statSync(rootDir);
    if (!stat.isDirectory()) {
        throw new Error(`Source path is not a directory: "${rootDir}". Please provide a directory, not a file.`);
    }
    // 2. Discover all files (recursive, with exclusions)
    const allFiles = discoverFiles(rootDir, rootDir);
    if (allFiles.length === 0) {
        throw new Error(`No scannable files found in "${rootDir}". Ensure the directory contains *.tsx, *.jsx, *.ts, *.js, *.css, *.scss, *.less, or *.json files.`);
    }
    ...
    for (const file of allFiles) {
        ...
        const classified = { ...file, category };
        switch (category) {
            case 'component':
                components.push(classified);
                break;
            case 'service':
                services.push(classified);
                break;
            case 'style':
                styles.push(classified);
                break;
            case 'config':
                configs.push(classified);
                break;
            case 'asset':
 ...
 */
function discoverFiles(dir, rootDir) {
    const results = [];
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (EXCLUDED_FILE_PATTERNS.some((re) => re.test(entry)))
            continue;
 ...
 */
function classifyFile(relPath, content, _rootDir) {
 ...
 */
function buildDependencyGraph(components, rootDir) {
 ...
 */
function normaliseImportPath(p) {
    return p
        .replace(/\\/g, '/')
 ...
 */
function detectProjectMeta(rootDir, allFiles) {
    const packageManager = detectPackageManager(rootDir);
    const packageJson = readPackageJson(rootDir);
    const buildTool = detectBuildTool(rootDir, packageJson);
    const hasTypeScript = allFiles.some((f) => f.path.endsWith('.ts') || f.path.endsWith('.tsx'));
    const allDeps = {
...
function detectBuildTool(rootDir, packageJson) {
    if (!packageJson)
        return 'unknown';
...
function configFileExists(rootDir, pattern) {
    try {
...
function detectUILibraries(allDeps) {
    const found = [];
    ...
    if ('tailwindcss' in allDeps)
        found.push('Tailwind CSS');
    return found;
}
const STATE_MANAGEMENT_MAP = {
    'redux': 'Redux',
...
function detectStateManagement(allDeps) {
    const found = new Set();
