 ...
 */
function extractDeclaredSignals(componentTs) {
    const names = new Set();
    let match;
 ...
 */
function extractTemplateSignalRefs(html) {
    const refs = new Set();
...
// ---------------------------------------------------------------------------
function fixNgModelBananaBox(html) {
    return html.replace(BANANA_NGMODEL_RE, (_match, signalName) => {
...
// ---------------------------------------------------------------------------
function ensureSignalCallSyntax(html, signalNames) {
    let result = html;
    ...
    // Fix @if (name) → @if (name())
    result = result.replace(/@if\s*\(\s*(\w+)\s*\)/g, (_match, name) => {
        if (signalNames.has(name)) {
            return `@if (${name}())`;
        }
        return _match;
    ...
    result = result.replace(/(@for\s*\([^)]*\bof\s+)(\w+)(\s*[;)])/g, (_match, prefix, name, suffix) => {
        if (signalNames.has(name)) {
    ...
    result = result.replace(/\[(\w+)\]="(\w+)"/g, (_match, prop, name) => {
...
// ---------------------------------------------------------------------------
function addMissingSignalDeclarations(componentTs, missingSignals) {
    ...
    if (!result.includes("from '@angular/core'")) {
        ...
        result = result.replace(/(import\s*\{[^}]*)(}\s*from\s*'@angular\/core')/, `$1, signal $2`);
    }
...
// ---------------------------------------------------------------------------
function ensureFormsModuleImport(componentTs, html) {
    ...
    if (!result.includes('FormsModule')) {
        ...
        if (result.includes("from '@angular/forms'")) {
            result = result.replace(/(import\s*\{[^}]*)(}\s*from\s*'@angular\/forms')/, `$1, FormsModule $2`);
        }
        else {
            result = `import { FormsModule } from '@angular/forms';\n${result}`;
        }
        // Add to @Component imports array
        result = result.replace(/(imports\s*:\s*\[)([^\]]*?)(\])/, (_match, open, existing, close) => {
...
function ensurePrimeNgImports(componentTs, html) {
    let result = componentTs;
    for (const [tag, info] of Object.entries(PRIMENG_TAG_TO_IMPORT)) {
        ...
        if (importsArray.includes(info.name))
            continue;
        ...
        if (hasImportsArray) {
            result = result.replace(/(imports\s*:\s*\[)([^\]]*?)(\])/, (_m, open, existing, close) => {
                const trimmed = existing.trim();
                if (trimmed.includes(info.name))
                    return `${open}${existing}${close}`;
                return trimmed
            ...
            result = result.replace(/(standalone\s*:\s*true\s*,)/, `$1\n  imports: [${info.name}],`);
        }
    }
    ...
    if (html.includes('<p-toast') || html.includes('</p-toast')) {
        if (!result.includes('MessageService')) {
            ...
            if (result.includes('providers:')) {
                result = result.replace(/(providers\s*:\s*\[)([^\]]*?)(\])/, (_m, open, existing, close) => {
...
// ---------------------------------------------------------------------------
function ensureChildComponentImports(componentTs, html, allComponents) {
    let result = componentTs;
...
// ---------------------------------------------------------------------------
function fixSubmitButtonPlacement(html) {
    ...
    if (footerWithSubmit.test(result)) {
        ...
        result = result.replace(/(<ng-template\s+pTemplate="footer">)/, '<!-- NOTA: Los botones de submit deben estar dentro del <form> para que (ngSubmit) funcione -->\n$1');
    }
    return result;
}
// ---------------------------------------------------------------------------
 ...
 */
export function fixSignals(components) {
...
//# sourceMappingURL=signal-fixer.js.map