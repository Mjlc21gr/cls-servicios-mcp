 ...
 */
function renderChild(child, bindings, indent) {
 ...
 */
function renderNode(node, bindings, indent) {
    ...
    if (PRIMENG_SELF_CLOSING.has(tag)) {
        if (tag === 'p-select' && node.children.length > 0) {
            ...
            if (options.length > 0) {
                ...
                const findPlaceholder = (children) => {
                    for (const c of children) {
                        if (typeof c !== 'object' || !('tag' in c))
                            continue;
                        if (c.tag === 'SelectValue') {
                            const ph = c.attributes.find(a => a.name === 'placeholder');
                            if (ph)
                                placeholder = typeof ph.value === 'string' ? ph.value : ph.value.expression;
                        }
                        if (c.children?.length)
                            findPlaceholder(c.children);
                    }
                };
 ...
 */
export function generateAngularTemplate(ir) {
    const bindings = [];
