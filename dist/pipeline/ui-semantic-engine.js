...
export function collapseShadcnSelect(html) {
    ...
    return html.replace(selectTreeRe, (_match, attrs, body) => {
        ...
        if (options.length === 0)
            return _match; // Can't extract options, leave as-is
...
// ---------------------------------------------------------------------------
export function collapseShadcnCard(html) {
    let result = html;
...
// ---------------------------------------------------------------------------
export function collapseMuiComponents(html) {
    let result = html;
    // MUI TextField → input pInputText
    result = result.replace(/<TextField([^>]*?)\/>/g, (_match, attrs) => {
...
// ---------------------------------------------------------------------------
export function collapseAntdComponents(html) {
    let result = html;
...
export function convertIconsToPI(html) {
    let result = html;
    for (const [reactIcon, piClass] of Object.entries(ICON_MAP)) {
        // Self-closing: <IconName className="..." />
        result = result.replace(new RegExp(`<${reactIcon}\\s+class(?:Name)?="([^"]*)"\\s*/>`, 'g'), `<i class="pi ${piClass} $1"></i>`);
        // Self-closing without className: <IconName />
        result = result.replace(new RegExp(`<${reactIcon}\\s*/>`, 'g'), `<i class="pi ${piClass}"></i>`);
        // With children (rare for icons): <IconName>...</IconName>
        result = result.replace(new RegExp(`<${reactIcon}[^>]*>[^<]*</${reactIcon}>`, 'g'), `<i class="pi ${piClass}"></i>`);
    }
    return result;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export function convertMotionToHtml(html) {
    let result = html;
...
// ---------------------------------------------------------------------------
export function convertToasterToToast(html) {
    let result = html;
...
// Badge → PrimeNG Tag
// ---------------------------------------------------------------------------
export function convertBadgeToTag(html) {
    let result = html;
    // <Badge variant="destructive">text</Badge> → <p-tag severity="danger" value="text" />
    result = result.replace(/<Badge\s+variant="destructive"[^>]*>([^<]*)<\/Badge>/g, '<p-tag severity="danger" value="$1" [rounded]="true"></p-tag>');
    result = result.replace(/<Badge\s+variant="secondary"[^>]*>([^<]*)<\/Badge>/g, '<p-tag severity="secondary" value="$1" [rounded]="true"></p-tag>');
    result = result.replace(/<Badge[^>]*>([^<]*)<\/Badge>/g, '<p-tag value="$1" [rounded]="true"></p-tag>');
    return result;
}
// ---------------------------------------------------------------------------
// Master function: Apply all semantic transformations
// ---------------------------------------------------------------------------
export function applySemanticUI(html) {
    let result = html;
...
//# sourceMappingURL=ui-semantic-engine.js.map