...
import type {
  ...
  ReactContextUsage,
  UILibraryUsage,
} from '../models/react-analysis.model.js';
...
const UI_LIBRARY_PATTERNS: Record<string, RegExp> = {
  '@mui/material': /^@mui\//,
...
export function analyzeReactComponent(sourceCode: string, fileName: string): ReactComponentAnalysis {
  const ast = parse(sourceCode, {
    sourceType: 'module',
    plugins: ['jsx', 'typescript', 'decorators-legacy'],
  });

  const stateHooks: ReactStateHook[] = [];
  const effects: ReactEffectHook[] = [];
  const props: ReactProp[] = [];
  const callbacks: ReactCallback[] = [];
  const memos: ReactMemo[] = [];
  const refs: ReactRef[] = [];
  const contexts: ReactContextUsage[] = [];
  const uiLibraries: UILibraryUsage[] = [];
    ...
    ImportDeclaration(path: NodePath<t.ImportDeclaration>) {
      ...
      for (const [libName, pattern] of Object.entries(UI_LIBRARY_PATTERNS)) {
        if (pattern.test(source)) {
          ...
          if (existing) {
            (existing.components as string[]).push(...components);
          } else {
...
function extractPropsFromParams(params: t.Node[], props: ReactProp[]): void {
...
function extractUseState(node: t.CallExpression, stateHooks: ReactStateHook[]): void {
  const parent = (node as unknown as { _parentPath?: { node: t.Node } })._parentPath?.node;
...
function extractUseRef(node: t.CallExpression, refs: ReactRef[]): void {
  const args = node.arguments;
...
function getTypeAnnotation(node: t.Node): string {
...
function extractTSType(node: t.TSType): string {
  ...
  if (t.isTSAnyKeyword(node)) return 'unknown'; // Prohibido any → unknown
...
function extractNodeSource(node: t.Node): string {
