...
function safeRewriteBody(body: string, ir: ComponentIR): string {
  ...
  for (const s of ir.state) {
    ...
    result = result.replace(
      new RegExp(`(?<!this\\.)(?<![.\\w])\\b${name}\\b(?!\\s*[:.(])(?!\\s*=[^=])`, 'g'),
      `this.${name}()`,
    );
  }
  ...
  for (const r of ir.refs) {
    if (r.isDomRef) {
      result = result.replace(
        new RegExp(`\\b${r.variableName}\\.current\\b`, 'g'),
        `this.${r.variableName}()?.nativeElement`,
      );
    }
  }
  ...
  for (const ctx of ir.contexts) {
    result = result.replace(
      new RegExp(`(?<!this\\.)\\b${ctx.variableName}\\.`, 'g'),
      `this.${ctx.variableName}.`,
    );
  }
  ...
  for (const svc of ir.angularServices) {
    for (const m of svc.methods) {
      ...
      if (inj) {
        ...
        result = result.replace(
          new RegExp(`(?<!this\\.)(?<![.\\w])\\b${methodName}\\(`, 'g'),
          `this.${injName}.${methodName}(`,
        );
      }
    }
  }
...
function rewriteTemplate(template: string, ir: ComponentIR): string {
  ...
  for (const name of signalNames) {
    result = result.replace(
      ...
      `[ngModel]="${name}()" (ngModelChange)="${name}.set($event)" [ngModelOptions]="{standalone: true}"`,
    );
  }
...
function generateComponentFile(ir: ComponentIR): string {
  ...
  if (ir.angularSignals.length > 0) coreImports.add('signal');
  if (ir.angularEffects.length > 0) coreImports.add('effect');
  if (ir.angularComputed.length > 0) coreImports.add('computed');
  if (ir.angularInjections.length > 0) coreImports.add('inject');
  if (ir.angularViewChildren.length > 0) coreImports.add('viewChild');
  if (ir.angularViewChildren.length > 0) coreImports.add('ElementRef');
  if (ir.props.length > 0) coreImports.add('Input');
  ...
  for (const method of ir.componentMethods) {
    ...
    const finalBody = safeBody.replace(
...
function generateSpecFile(ir: ComponentIR): string {
  const className = toPascalCase(ir.componentName);
  const kebabName = toKebabCase(ir.componentName);
  const lines: string[] = [];
...
function generateTailwindConfig(ir: ComponentIR): string {
  const kebabName = toKebabCase(ir.componentName);
  const lines: string[] = [];
