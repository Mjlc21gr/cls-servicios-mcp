...
const MUI_TO_PRIMENG: Record<string, PrimeNgMapping> = {
  'Button': {
...
const HTML_TO_PRIMENG: Record<string, PrimeNgMapping> = {
  'input[type="text"]': {
    primeComponent: 'InputText',
    primeModule: 'InputTextModule',
    primeImport: 'primeng/inputtext',
 ...
 */
function cleanPrimeNgAttributes(html: string): string {
  let result = html;
 ...
 */
export function convertToPrimeNg(
  html: string,
  detectedUiLibraries: readonly string[]
): PrimeNgConversionResult {
  let converted = html;
  const modules = new Map<string, PrimeNgModuleImport>();
 ...
 */
function convertReactPropsToAngular(html: string): string {
  let result = html;
 ...
 */
function injectSbClasses(html: string): string {
  let result = html;
  ...
  result = result.replace(
    /<h1(?![^>]*class=)/g,
    '<h1 class="sb-heading-1"'
  );
  result = result.replace(
    /<h2(?![^>]*class=)/g,
    '<h2 class="sb-heading-2"'
  );
  result = result.replace(
    /<h3(?![^>]*class=)/g,
    '<h3 class="sb-heading-3"'
  );
 ...
 */
function validateAccessibility(html: string): string[] {
  const warnings: string[] = [];
 ...
 */
export function generatePrimeNgImports(modules: readonly PrimeNgModuleImport[]): string {
 ...
 */
export function generateSbPrimeNgTheme(): string {
  return `// ═══════════════════════════════════════════════════════════════
