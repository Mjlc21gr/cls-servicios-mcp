...
function handleMessage(raw: string): void {
  ...
  switch (method) {
    ...
    case 'tools/call':
      void callTool(id, params as { name: string; arguments?: Record<string, unknown> });
      break;
  ...
  try {
    ...
    switch (params.name) {
      ...
      case 'convert_react_to_angular': {
        const v = validateInput(args.sourceCode as string);
        if (!v.isValid) { text = JSON.stringify({ error: v.errors }); break; }
        const ir = parseReactComponent(v.sanitizedCode ?? (args.sourceCode as string));
      ...
      case 'analyze_react_component': {
        const ir = parseReactComponent(args.sourceCode as string);
