 ...
 */
function isClassComponent(source: string): boolean {
 ...
 */
function extractStateVars(source: string): Array<{ name: string; defaultValue: string }> {
  const stateAssignMatch = source.match(/this\.state\s*=\s*\{([^}]+)\}/);
  if (!stateAssignMatch) return [];
 ...
 */
function generateUseStateDeclarations(stateVars: Array<{ name: string; defaultValue: string }>): string {
  return stateVars
 ...
 */
function replaceSetStateCalls(
  source: string,
  stateVars: Array<{ name: string; defaultValue: string }>,
  ...
  for (const v of stateVars) {
    const funcPattern = new RegExp(
      `this\\.setState\\(\\s*(\\w+)\\s*=>\\s*\\(\\{[^}]*${v.name}\\s*:\\s*([^,}]+)[^)]*\\}\\)\\s*\\)`,
      'g',
    );
  ...
  for (const v of stateVars) {
    const simplePattern = new RegExp(
      `this\\.setState\\(\\s*\\{[^}]*${v.name}\\s*:\\s*([^,}]+)[^}]*\\}\\s*\\)`,
      'g',
    );
    result = result.replace(simplePattern, (_match, value: string) => {
 ...
 */
function convertLifecycleMethods(source: string): { result: string; notes: string[] } {
  let result = source;
  const notes: string[] = [];

  // componentDidMount() { ... } → useEffect(() => { ... }, []);
  result = result.replace(
    /componentDidMount\s*\(\s*\)\s*\{([\s\S]*?)\n\s*\}/,
    (_match, body: string) => {
  ...
  result = result.replace(
    /componentDidUpdate\s*\(\s*\w*\s*\)\s*\{([\s\S]*?)\n\s*\}/,
    (_match, body: string) => {
 ...
 */
function convertPropsAccess(source: string): { result: string; propNames: string[]; notes: string[] } {
 ...
 */
function extractRenderBody(source: string): string | null {
 ...
 */
function unwrapReactMemo(source: string): { result: string; unwrapped: boolean } {
  const memoPattern = /(?:export\s+default\s+)?React\.memo\(\s*(\w+)\s*\)/g;
  const hasMemo = memoPattern.test(source);
  if (!hasMemo) return { result: source, unwrapped: false };
 ...
 */
function unwrapForwardRef(source: string): { result: string; unwrapped: boolean } {
  const forwardRefPattern = /React\.forwardRef\(/;
  if (!forwardRefPattern.test(source)) return { result: source, unwrapped: false };
...
export function convertClassToFunctional(source: string): ClassComponentConversion {
  ...
  const hooksNeeded: string[] = [];
  if (stateVars.length > 0) hooksNeeded.push('useState');
  if (effectMatches && effectMatches.length > 0) hooksNeeded.push('useEffect');
