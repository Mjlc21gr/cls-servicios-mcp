...
interface CorrectionReport {
  ...
  importPathsFixed: number;
  templateFixes: number;
}
 ...
 */
export function validateClassContext(
  ...
  let totalReport: CorrectionReport = { thisFixCount: 0, duplicatesRemoved: 0, importPathsFixed: 0, templateFixes: 0 };
...
function fixThisScope(ts: string): { result: string; fixCount: number } {
  let fixCount = 0;
  ...
  if (propertyNames.size === 0) return { result: ts, fixCount: 0 };
  ...
  for (const prop of propertyNames) {
    ...
    const propAccessRe = new RegExp(
      `(?<!this\\.)(?<!readonly\\s)(?<![.\\w])\\b${prop}\\.(length|filter|map|find|some|every|reduce|forEach|includes|indexOf|slice|concat|join|push|pop|shift|unshift|splice)\\b`,
      'g',
    );
    ...
    const bareReadRe = new RegExp(
      `(?<!this\\.)(?<!readonly\\s+)(?<![.\\w])\\b${prop}\\b(?!\\s*[(.=:])(?!\\s*=\\s*(?:signal|computed|inject|input|output|viewChild))`,
      'g',
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\s*(?:readonly\s+)?(?:@Input\(\)\s+)?\w+\s*[:=]/.test(line) && !inMethodBody) continue;
      // Skip import lines
...
function consolidateIdentifiers(ts: string): { result: string; removedCount: number } {
  let removedCount = 0;
  let result = ts;
  ...
  for (const name of signalNames) {
    if (injectNames.has(name)) {
      ...
      result = result.replace(
        new RegExp(`((?:private\\s+)?(?:readonly\\s+)?)${name}(\\s*=\\s*inject\\()`, 'g'),
        `$1${newName}$2`,
      );
      // Update all references to the old inject name
      result = result.replace(
        new RegExp(`this\\.${name}\\.(?!set\\(|update\\(|\\(\\))`, 'g'),
        `this.${newName}.`,
      );
      removedCount++;
    }
  ...
  for (const line of cleanedLines) {
    if (skipUntilBrace) {
      ...
      if (line.includes('}')) braceDepth--;
      if (braceDepth <= 0) { skipUntilBrace = false; braceDepth = 0; }
      continue;
    }
    ...
    if (methodMatch && !line.includes('constructor') && !line.includes('=')) {
      if (seenMethods.has(methodMatch[1])) {
        removedCount++;
        if (line.includes('{')) { skipUntilBrace = true; braceDepth = 1; }
        continue;
      }
...
function sanitizeTemplate(html: string): { result: string; fixCount: number } {
  let fixCount = 0;
  let result = html;
  ...
  result = result.replace(backtickBindingRe, (_match, attr, inner) => {
    ...
    if (parts.length === 1) return `[${attr}]="'${inner}'"`;
    let expr = '';
    for (let i = 0; i < parts.length; i++) {
      if (i % 2 === 0) {
        if (parts[i]) expr += (expr ? " + " : "") + `'${parts[i]}'`;
      } else {
        expr += (expr ? " + " : "") + `(${parts[i]})`;
      }
    }
  ...
  result = result.replace(/(\s)(\w+)="`([^`]*)`"/g, (_match, space, attr, inner) => {
    fixCount++;
    const parts = inner.split(/\$\{([^}]+)\}/);
    if (parts.length === 1) return `${space}${attr}="${inner}"`;
    let expr = '';
    for (let i = 0; i < parts.length; i++) {
      if (i % 2 === 0) {
        if (parts[i]) expr += (expr ? " + " : "") + `'${parts[i]}'`;
 ...
 */
function fixBindingDoubleQuotes(html: string): string {
...
function fixAsyncAwait(ts: string): string {
  let result = ts;
...
function removeDuplicateClasses(ts: string): string {
  const classMatches = [...ts.matchAll(/^export class \w+Component/gm)];
  if (classMatches.length <= 1) return ts;
...
function findClassEnd(ts: string, classStart: number): number {
  let braceCount = 0;
  let inClass = false;
  for (let i = classStart; i < ts.length; i++) {
    if (ts[i] === '{') { braceCount++; inClass = true; }
    else if (ts[i] === '}') { braceCount--; if (inClass && braceCount === 0) return i; }
  }
  return -1;
}

function truncateAfterClassEnd(ts: string): string {
  const lines = ts.split('\n');
  let lastClosingBrace = -1;
  for (let i = lines.length - 1; i >= 0; i--) {
    if (lines[i].trim() === '}') { lastClosingBrace = i; break; }
  }
  if (lastClosingBrace === -1) return ts;
...
function cleanImports(ts: string): string {
  let result = ts;
...
function cleanMethodBodies(ts: string): string {
  let result = ts;
