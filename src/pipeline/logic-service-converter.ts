...
export interface GeneratedService {
  readonly fileName: string;      // kebab-case.service.ts
  readonly className: string;     // PascalCaseService
  readonly content: string;       // Full TypeScript source
}
 ...
 */
export function convertHookToService(
  hookFile: ScannedFile,
  typesContent: string,
...
interface HookAnalysis {
  readonly stateVars: StateVar[];
  readonly effects: EffectBlock[];
...
function analyzeHookBody(body: string): HookAnalysis {
...
function generateServiceContent(
  ...
  if (analysis.stateVars.length > 0) lines.push('');
  ...
  for (const method of analysis.methods) {
    ...
    if (method.name.includes('Stats') || method.name.includes('stats')) continue;
    ...
    for (const sv of analysis.stateVars) {
      cleanBody = cleanBody.replace(
        new RegExp(`\\b${sv.setter}\\(`, 'g'),
        `this.${sv.name}.set(`,
      );
      // Replace state reads: xxx → this.xxx()
      cleanBody = cleanBody.replace(
        new RegExp(`(?<!this\\.)\\b${sv.name}\\b(?!\\s*[(.=])`, 'g'),
        `this.${sv.name}()`,
      );
    }
