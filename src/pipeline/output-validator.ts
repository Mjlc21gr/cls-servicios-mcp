...
const REQUIRED_DEPS: readonly string[] = [
  '@angular/core',
  '@angular/common',
  '@angular/router',
  'primeng',
 ...
 */
function validateComponentCompleteness(
  ...
  for (const [filePath, content] of files) {
    ...
    if (filePath === 'src/app/app.component.ts') continue;

    // Skip components with inline templates (template: `...`)
    if (content.includes("template:") && !content.includes("templateUrl:")) continue;
 ...
 */
function validateStandaloneComponents(
  files: ReadonlyMap<string, string>,
): readonly ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
function validateNoPrimeNGRequired(
  files: ReadonlyMap<string, string>,
): readonly ValidationIssue[] {
  const issues: ValidationIssue[] = [];
