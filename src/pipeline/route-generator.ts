 ...
 */
const ROUTE_COMPONENT_RE =
  /<Route\s[^>]*?path\s*=\s*["']([^"']+)["'][^>]*?component\s*=\s*\{(\w+)\}/g;

const ROUTE_ELEMENT_RE =
  /<Route\s[^>]*?path\s*=\s*["']([^"']+)["'][^>]*?element\s*=\s*\{<(\w+)/g;

/** Also handle reversed attribute order: element before path */
const ROUTE_ELEMENT_REV_RE =
  /<Route\s[^>]*?element\s*=\s*\{<(\w+)[^>]*?path\s*=\s*["']([^"']+)["']/g;

const ROUTE_COMPONENT_REV_RE =
  /<Route\s[^>]*?component\s*=\s*\{(\w+)\}[^>]*?path\s*=\s*["']([^"']+)["']/g;
...
function collectJsxRoutes(
  ...
  const patterns: Array<{ re: RegExp; pathIdx: number; compIdx: number }> = [
...
const CREATE_ROUTES_FROM_ELEMENTS_RE =
  /createRoutesFromElements\s*\(/;

const OBJECT_ROUTE_RE =
  /\{\s*path\s*:\s*["']([^"']+)["'][^}]*?(?:component|element)\s*:\s*(?:<\s*)?(\w+)/g;

function extractObjectRoutes(
  scannedProject: ScannedProject,
  transformedComponents: ReadonlyMap<string, TransformedComponent>,
  moduleName?: string,
): GeneratedRoute[] {
...
function buildDefaultRoute(
  ...
  if (transformedComponents.size === 0) return [];
...
interface ResolvedComponent {
  ...
  route: (path: string) => GeneratedRoute;
}
...
function resolveComponent(
  ...
  if (direct) return wrapResolved(direct, moduleName);
 ...
 */
function resolveComponentFromFile(
  filePath: string,
  transformedComponents: ReadonlyMap<string, TransformedComponent>,
  moduleName?: string,
): ResolvedComponent | null {
...
function wrapResolved(comp: TransformedComponent, moduleName?: string): ResolvedComponent {
  return {
    component: comp,
    route: (path: string): GeneratedRoute => ({
      path,
 ...
 */
function buildImportPath(comp: TransformedComponent, moduleName?: string): string {
...
function renderRoutesFile(routes: GeneratedRoute[]): string {
  const lines: string[] = [
  ...
  for (const route of routes) {
    ...
    lines.push(
      `    loadComponent: () => import('${route.componentPath}').then(m => m.${route.componentName}Component),`,
    );
    lines.push(`  },`);
  }
