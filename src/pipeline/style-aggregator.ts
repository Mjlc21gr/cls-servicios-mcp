 ...
 */
const JS_TO_TAILWIND: Record<string, Record<string, string>> = {
 ...
 */
function spacingToTailwind(value: string): string | null {
 ...
 */
function jsStyleToTailwindClass(prop: string, value: string): string | null {
  // Direct mapping
  ...
  if (spacingProps[prop]) {
    ...
    if (tw) return `${spacingProps[prop]}-${tw}`;
  }
  ...
  if (prop === 'borderRadius') {
    if (value === '50%' || value === '9999px') return 'rounded-full';
    const tw = spacingToTailwind(value);
 ...
 */
function jsStyleToCSS(jsStyle: string): string {
  const lines: string[] = [];
 ...
 */
function jsStyleToTailwindAndCSS(jsStyle: string): { tailwindClasses: string[]; remainingCSS: string } {
 ...
 */
function scopeUnderHost(css: string): string {
 ...
 */
function convertCssModuleRefs(html: string): string {
 ...
 */
function lessToScss(lessContent: string): string {
 ...
 */
function resolveStyleFile(
  importPath: string,
  componentPath: string,
  styleFiles: readonly { path: string; content: string }[],
): string | undefined {
 ...
 */
function extractStyledComponents(source: string): string {
  const rules: string[] = [];
  let match: RegExpExecArray | null;
 ...
 */
function extractEmotionCSS(source: string): string {
  const rules: string[] = [];
  let match: RegExpExecArray | null;
 ...
 */
function extractMuiSx(source: string): string {
  const rules: string[] = [];
  let match: RegExpExecArray | null;
 ...
 */
function generateGlobalStyles(_options?: MigrationOptions): string {
  return `/* =================================================================
...
export function aggregateStyles(
  ...
  for (const [key, component] of transformedComponents) {
    ...
    const originalFile = scannedProject.components.find(
      (f) => f.path === component.originalPath,
    );
    ...
    while ((importMatch = cssImportRe.exec(reactSource)) !== null) {
      ...
      const styleContent = resolveStyleFile(
        importPath,
        component.originalPath,
        scannedProject.styles,
      );
      if (styleContent) {
