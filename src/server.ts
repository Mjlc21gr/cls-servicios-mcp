  ...
  server.tool(
    ...
    async ({ table, limit }) => {
      try {
        ...
        if (!isDbConfigured()) {
          return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', message: 'DB no configurada. Necesitas clientId y clientSecret.' }) }], isError: true };
        }
        const tableMap: Record<string, string> = { intentos: 'INTENTOS', errores: 'ERRORES', patches: 'PATCHES', ml_seguimiento: 'ML_SEGUIMIENTO' };
        ...
        switch (table) {
          ...
          case 'ml_seguimiento': data = await getPendientes(); break;
          default: {
            ...
            data = res.ok ? await res.json() as unknown[] : [];
          }
        }
