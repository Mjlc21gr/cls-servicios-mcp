...
async function runConvertPipeline(sourceCode: string) {
  ...
  try {
    ...
    if (fixResult.errorsFixed > 0) {
      ...
      if (tsFile) artifact.componentFile = tsFile.content;
      if (htmlFile) artifact.templateFile = htmlFile.content;
      if (scssFile) artifact.scssFile = scssFile.content;
    }
  } catch {
    ...
    async (args) => {
      try {
        ...
        const llmConfig = process.env['GEMINI_API_KEY']
          ? { url: 'https://generativelanguage.googleapis.com/v1beta/models', model: 'gemini-2.5-flash', apiKey: process.env['GEMINI_API_KEY'], type: 'gemini' as const }
          : undefined;
