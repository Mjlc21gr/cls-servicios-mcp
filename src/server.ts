...
export function createServer(): McpServer {
  ...
  server.tool(
    'analyze_appscript',
    `Analiza un proyecto Google Apps Script y extrae su estructura completa:
    ...
    async ({ files, projectName }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', message: `Error al analizar Apps Script: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    'convert_appscript_to_angular',
    `Convierte un proyecto Apps Script analizado a componentes Angular Standalone CLS.
    ...
    async ({ files, projectName, moduleName }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', message: `Error en conversión Apps Script: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    'generate_appscript_services',
    `Genera servicios Angular (.service.ts) a partir de funciones server-side de Apps Script.
    ...
    async ({ files, projectName, moduleName }) => {
      try {
        ...
        if (!serviceResult) {
          return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'info', message: 'No se detectaron funciones server-side, llamadas externas ni PropertiesService.' }) }] };
        }
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', message: `Error generando servicio: ${error instanceof Error ? error.message : String(error)}` }) }],
          isError: true,
        };
      }
    },
  ...
  server.tool(
    'read_appscript_from_gcp',
    `Lee un proyecto Google Apps Script completo usando una cuenta de servicio de GCP.
  ...
  server.tool(
    'migrate_appscript_from_gcp',
    `Lee un proyecto Apps Script desde GCP por script ID y lo migra completo a Angular CLS.
  ...
  server.tool(
    'migrate_appscript_full_project',
    `Migra un proyecto Apps Script completo a un proyecto Angular 20 + PrimeNG 19 listo para ejecutar.
  ...
  server.tool(
    'smart_migrate',
    `Tool unificado de migración a Angular CLS con detección automática del tipo de fuente.
    ...
    async ({ moduleName, scriptId, sourceCode, fileName, files }) => {
      try {
        ...
        if (files && Object.keys(files).length > 0) {
          ...
          if (detection.type === 'react') {
            ...
            for (const [fName, fCode] of Object.entries(files)) {
              if (!/\.(tsx|jsx)$/.test(fName)) continue;
              try {
          ...
          return {
            content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', detectedType: 'unknown', confidence: detection.confidence, signals: detection.signals, message: 'No se pudo determinar el tipo de fuente. Usa scriptId para Apps Script de GCP, o asegúrate de que los archivos tengan extensiones .tsx/.jsx (React) o .gs/.html (Apps Script).' }) }],
            isError: true,
          };
        }
        ...
        if (sourceCode) {
          ...
          return {
            content: [{ type: 'text' as const, text: JSON.stringify({ status: 'error', detectedType: 'unknown', confidence: detection.confidence, signals: detection.signals, message: 'No se pudo determinar si el código es React o Apps Script. Incluye el fileName para ayudar a la detección.' }) }],
            isError: true,
          };
        }
...
3. Si falla, reporta exactamente qué falló y por qué`,
  ...
  server.prompt(
    'migrate_appscript_to_angular',
    `Prompt de orquestación para migrar un proyecto Apps Script completo a Angular CLS.`,
    {
      projectName: z.string().describe('Nombre del proyecto Apps Script'),
      moduleName: z.string().describe('Nombre del módulo destino'),
    },
