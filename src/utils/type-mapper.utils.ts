...
const REACT_TO_TS_TYPE_MAP: Record<string, string> = {
...
export function mapHttpMethodFromFetchCall(fetchBody: string): 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' {
  ...
  if (fetchBody.includes('.post(') || fetchBody.includes('.post ')) return 'POST';
  if (fetchBody.includes('.put(') || fetchBody.includes('.put ')) return 'PUT';
  if (fetchBody.includes('.delete(') || fetchBody.includes('.delete ')) return 'DELETE';
  if (fetchBody.includes('.patch(') || fetchBody.includes('.patch ')) return 'PATCH';
  return 'GET';
}
